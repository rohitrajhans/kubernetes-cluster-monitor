...
const (
	annotationKey       = "sidecar-injector-webhook.rohit.in/inject"
...
func RetrieveExpression(obj interface{}) string {
	...
	rcvr := obj.(*v1alpha1.Receiver).DeepCopy()

	labelMap := rcvr.Spec.LabelSelector.Labels
	expression := ""
	...
	mExpr := rcvr.Spec.LabelSelector.Expression
...
func (t *TestHandler) ObjectCreated(obj interface{}, c *Controller) {
	...
	var nspaces = make([]string, len(rcvr.Spec.Namespace)+1)
	if len(rcvr.Spec.Namespace) != 0 {
		for i, val := range rcvr.Spec.Namespace {
	...
	var groupLabels = make([]string, len(rcvr.Spec.GroupLabel))
	groupLabelString := ""
	for i, val := range rcvr.Spec.GroupLabel {
		groupLabels[i] = val
		groupLabelString = groupLabelString + val + "--"
	}
	groupLabelString = strings.TrimRight(groupLabelString, "-")
	var logFreq string
	if rcvr.Spec.LogFrequency == 0 {
		logFreq = "10"
	} else {
		logFreq = strconv.Itoa(rcvr.Spec.LogFrequency)
	}
	// store action to be taken
	action := rcvr.Spec.Action
